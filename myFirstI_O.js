...
// (\n) it contains to the console (stdout), similar to running 
// cat file | wc -l.
...
// end with 'Sync'. To read a file, you'll need to use fs.readFileSync('/path/to/file'). 
...
// and that '\n' can be used as a delimiter. Note that the test file does not 
// have a newline character ('\n') at the end of the last line, so using this 
// method you'll end up with an array that has one more element than the 
// number of newlines.
...
//readFileSync doesn't take a callback! Because it's Synchornous / Blocking!
...
//---------Alternative Correct Solution using .split()
